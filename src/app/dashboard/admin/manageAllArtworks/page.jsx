...
import DeleteArtworkButton from '@/app/components/dashboard/DeleteArtworkButton';
...
const ManageArtworksPage = async ({ searchParams }) => {
    ...
    const totalItems = allArtworks.length;
                            ...
                            <table className="w-full text-left border-collapse">
                                ...
                                <tbody className="divide-y divide-gray-800/60 text-sm text-gray-300">
                                    {paginatedArtworks.map((art) => (
                                        <tr key={art._id || art.id} className="hover:bg-gray-900/30 transition-colors">
                                            <td className="p-4 font-semibold text-gray-200">
                                                {art.title || 'Untitled'}
                                            </td>
                                            <td className="p-4 text-gray-400">
                                                {art.artistName || art.artistEmail || 'Unknown Artist'}
                                            </td>
                                            <td className="p-4 font-bold text-pink-500">
                                                ${art.price ? Number(art.price).toFixed(2) : 'Free'}
                                            </td>
                                            <td className="p-4 text-center">
                                                <DeleteArtworkButton artworkId={art._id || art.id} title={art.title} />
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
